...
class RingSubmitterSpec extends FlatSpec {
  ...
  case class Order(
      ...
      amountS: String,
      amountB: String,
      tokenSFeePercentage: String,
      tokenBFeePercentage: String,
      walletAddr: String,
      balanceS: String,
      balanceB: String,
      feeAmount: String,
      feePercentage: String,
      dualAuthSignAlgorithm: String,
      dualAuthAddr: String,
      allOrNone: Boolean,
      validSince: String,
      validUntil: String,
      walletSplitPercentage: String,
      version: String,
      tokenRecipient: String,
      feeToken: String,
      waiveFeePercentage: String,
      hash: Array[Byte],
      sig: String,
      dualAuthSig: String
  )
  ...
  * */
  ...
  "submitRings" should "" in {
    val amountS = "100000000000000000000"
    val amountB = "380000000000000000000"
    info(number2hex(amountS))
    info(number2hex(amountB))

    //    val order1 = Order(
    //      owner = "0x8f6a0679ed3767b2dc27fe28e159fb5ac02b725b",
    //      tokenS = "0xe8abb09646f9d774d8498e844c699a8f6bcd4964",
    //      tokenB = "0xe90aa62b8239ed12cbb37f976e774e0ca75668f0",
    //      amountS = BigInt.apply("100000000000000000000").toString(),
    //    )

    //    val sig = Hex.toHexString(func.encodeSignature())
    //    val name = func.encodeSignature()
    //    val input = getInputBytes(originInput)
    //    val result = func.decode(input)
    //    info(abi.toString)
  }

  private def number2hex(number: String): String = {
    BigInt(number).toHex.replace("0x", "")
  }
}
